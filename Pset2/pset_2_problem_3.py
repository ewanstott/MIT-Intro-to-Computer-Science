...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 19 08:01:57 2023
...
Write a program that uses these bounds and bisection search (for more info check out the Wikipedia page on bisection search)
 ...
 Psuedocode:
     Guess minimum monthly payment (high+low)/2
         if guess too high:
             adjust high guess to what guess was
        if guess too low:
            adjust low guess to what guess was 
        


How to deal with float values? Answer needs to include cents 
abs
...
low = tempBalance/12 
high = (tempBalance*(1+monthlyInterestRate)**12)/12.0 
epsilon = 0.01

while abs(balance) > epsilon: 
    monthlyPaymentGuess = (high + low)/2
    balance = tempBalance
    for month in range(12):
        balance = balance - monthlyPaymentGuess + ((balance - monthlyPaymentGuess)*monthlyInterestRate)
    if balance > epsilon:
        low = monthlyPaymentGuess
    elif balance < -epsilon: 
        high = monthlyPaymentGuess
    else:
        break
print("Lowest Payment: ", round(monthlyPaymentGuess, 2))
